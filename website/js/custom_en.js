...
var cardtemplate = '<div class="col1-3 white grey-area-last element home"><p class="blockquotetile">/quote/</p>' +
    ...
    '<div class="sharelayout"><img class="share fb" alt="Facebook" title="share on Facebook" src="../img/facebook55.png">' +
    '<img class="share google" id="/id/" alt="Google+" title="share on Google+" src="../img/google1.png"><img class="share mail" alt="E-Mail" title="send via E-Mail" src="../img/symbol20.png">' +
    '<img class="share twitter" alt="Twitter" title="share on Twitter" src="../img/twitter1.png"><img class="fav" id="fav" alt="Favorite" title="add to favorites" src="../img/stars59.png"></div></div></div>';

var loadmorecard = '<div class="clearfix col1-1 element home"><p class="loadmore" id="loadmore">load more</p></div>';

var nofavscard = '<div class="col1-1 nofavscard combi white grey-area-last element"><p class="blockquotetile">You haven&#39;t saved any favorites yet...</p>' +
    '<div class="grey-area-fullwidth last smaller clearfix bottom-area"><p class="small">Hint: Favorites are being saved locally in the browser cache. ' +
    'You&#39;re saved quotes can be lost when deleting browser data/caches!</p></div></div>';

var nosearchresultscard = '<div class="col1-1 nothingfound combi white grey-area-last element"><p class="blockquotetile">Nothing found, sorry...</p>' +
    '<div class="grey-area-fullwidth last smaller clearfix bottom-area"><p class="small">Try again with different search terms.</p></div></div>';
    ...
    if ($('.nav').attr("class").indexOf("open") <= -1) {
        ...
        contactData.save(null, {
            success: function () {
                ...
                showDialog({
                    title: 'Many thanks!',
                    text: 'Your suggestion was successfully transmitted.',
                    positive: {
                        title: 'OK',
                        onClick: function (e) {
            ...
            error: function (gameScore, error) {
                ...
                showDialog({
                    title: 'Error!',
                    text: 'Unfortunately your suggestion couldn&#39;t be transmitted. Please try again.<br><br>Error: '
                    + error.message,
                    positive: {
                        title: 'OK',
                        onClick: function (e) {
...
$(document).on("click", ".sharelayout img", function () {
    ...
    if ($(this).attr("class").indexOf("fb") > -1) {
        ...
        shareText = "\"" + quote + "\" -" + source.replace("THE BIBLE", "");
        ...
        if (localStorage.getItem('fav' + quote) === null) {
            ...
            if (!existingFavs) {
                showDialog({
                    title: 'Hint',
                    text: 'Favorites are being saved locally in the browser cache. ' +
                    'You&#39;re saved quotes can be lost when deleting browser data/caches!',
                    positive: {
                        title: 'OK',
                        onClick: function (e) {
...
$('#search-icon').click(function (event) {
    if ($search_expandable.val() !== "") {
        ...
        if ($title.text().indexOf('Fav') > -1) {
            searchFavs();
        } else if ($title.text().indexOf('Browse') > -1) {
            searchQuotes();
        }
    } else {
        ...
        success: function (results) {
            if (results.length > 0) {
                for (var i = 0; i < results.length; i++) {
                    ...
                    var quote = object.get('en');
                    var source = object.get('verse').replace('<b>', '').replace('</b>', '');
                    if (localStorage.getItem('fav' + quote) !== null) {
                        $fab_fav_img.attr('src', '../img/fav_full_square.png');
                        $fab_fav_img.attr('title', 'remove from favorites');
                        $('#favorite').attr('title', 'remove from favorites');
                    } else {
                        $fab_fav_img.attr('src', '../img/fav_empty_square.png');
                        $fab_fav_img.attr('title', 'add to favorites');
    ...
    for (var i = 0, len = localStorage.length; i < len; ++i) {
        if (localStorage.key(i).indexOf('fav') > -1) {
            ...
            card = card.replace('src="../img/stars59.png"', 'src="../img/star7.png"')
                .replace('title="add to favorites"', 'title="remove from favorites"')
...
function searchQuotes() {
    ...
    for (var i = 0; i < searchTerms.length; i++) {
        ...
        quotequery.contains("en_search", currentterm);
        var sourcequery = new Parse.Query(ParseObj);
        sourcequery.contains("verse_search", currentterm);
    ...
    mainQuery.find({
        success: function (results) {
            foundFavs = true;
            for (var i = 0; i < results.length; i++) {
                if ($alpha.find('.element .blockquotetile:contains("' + results[i].get("en") + '")').length > 0) {
                } else {
                    var card = cardtemplate.replace('/quote/', results[i].get("en"))
                        .replace('/source/', results[i].get("verse").replace("<b>", "").replace("</b>", ""))
                        .replace('/id/', 'googleplus' + i);
                    ;
                    if (localStorage.getItem('fav' + results[i].get("en")) !== null) {
                        card = card.replace('src="../img/stars59.png"', 'src="../img/star7.png"')
                            .replace('title="add to favorites"', 'title="remove from favorites"')
                            .replace('white', 'color');
                    }
                    $alpha.append(card);
                    gRenderShare(results[i].get("en"), results[i].get("verse").replace("<b>", "").replace("</b>", ""), 'googleplus' + i);
                    $('.sharelayout').transition({y: '40px', duration: 0});
                    $('.sharelayout').hide();
                }
            }
        ...
        success: function (results) {
            for (var i = 0; i < results.length; i++) {
                ...
                if (quote) {
                    ...
                    showDialog({
                        title: 'Error!',
                        text: 'Unfortunately today&#39;s quote couldn&#39;t be loaded. Please try again!',
                        positive: {
                            title: 'OK',
                            onClick: function (e) {
